...
const EXPECTED_PLUGIN_DESCRIPTION := "Template for AeroBeat internal cosmetic repos. Shared cosmetic resources built on aerobeat-asset-core for current v1 work."
const EXPECTED_TESTBED_NAME := "AeroBeat Internal Cosmetic Testbed"
...
func test_readme_keeps_v1_internal_cosmetic_template_truth() -> void:
	var readme_text := _read_repo_file(README_PATH)
	assert_true(readme_text.contains("official template for creating **internal cosmetic** repositories"), "README should state that this repo is an internal cosmetic template")
	...
	assert_true(readme_text.contains("internal/system cosmetic work"), "README should preserve the downscoped internal-only cosmetic stance")
	assert_true(readme_text.contains("controlled avatar/profile presentation"), "README should tie cosmetics to controlled product presentation")
	assert_true(readme_text.contains("UGC or package-local gameplay cosmetic swap story"), "README should explicitly reject the old UGC/package-local cosmetic framing")
...
func test_plugin_cfg_description_stays_template_specific() -> void:
	...
	assert_eq(config.get_value("plugin", "name", ""), "AeroBeat Internal Cosmetic Template", "plugin.cfg name should stay stable")
	assert_eq(
		config.get_value("plugin", "description", ""),
		EXPECTED_PLUGIN_DESCRIPTION,
		"plugin.cfg description should remain aligned with the template's narrow v1 cosmetic-lane contract"
	)
...
func test_hidden_testbed_name_stays_internal_template_specific() -> void:
	...
	assert_eq(
		...
		"hidden workbench name should stay aligned with the internal cosmetic template contract"
	)
